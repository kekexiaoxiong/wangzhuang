...
<script>
import { findRemainMediasource } from "@/api/advertisingAnalysis";
...
export default {
  ...
  methods: {
    ...
    //获取数据
    //findRemainMediasource(appId, country, dateTime)
    fetchData(data) {
     this.$refs.childHide.loginHide = false; //隐藏子组件不要的下拉菜单
      ...
      findRemainMediasource(this.appId, this.country, this.dateTime).then(
        response => {
